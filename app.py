...
if mode == "Forward":
    ...
    if st.button("Predict Forward"):
        ...
        st.write(f"**Postseason Efficiency ({round(eff, 3)})**: A custom metric based on conference tournament performance. Higher values suggest stronger conference postseason play.")
...
# -----------------------
    ...
    ncaa_tournament = st.number_input("NCAA Tournament (0 or 1)", min_value=0, max_value=1, value=1, step=1)
    conf_rank = st.number_input("Conference Rank", min_value=1, max_value=25, value=5, step=1)
    postseason_eff = st.number_input("Postseason Efficiency", min_value=0.0, max_value=5.0, value=1.5, step=0.01)
